...
/**
 * @author 云岩
 * @description 状态迁移动作处理步骤
 * @date 2021/5/25 下午2:38
 */
public interface StateActionStep<T, C> {
     ...
     * 状态动作方法，主要状态迁移逻辑
     */
    ServiceResult<T,C> action(OrderStateEnum nextState, StateContext<C> context) throws Exception;
    /**
     * 状态数据持久化
     */
    ServiceResult<T,C> save(OrderStateEnum nextState, StateContext<C> context) throws Exception;
    /**
     * 状态迁移成功，持久化后执行的后续处理
     */
